...
class CostLedger:
    ...
    def record_llm(self, property_id: str, pms: str, tier: str, cost: float, model: str, tokens: int) -> None:
        """Record an LLM cost entry."""
        self._insert(property_id, pms, tier, "llm", cost, {"model": model, "tokens": tokens})

    def record_vision(self, property_id: str, pms: str, tier: str, cost: float, model: str) -> None:
        """Record a Vision cost entry."""
        self._insert(property_id, pms, tier, "vision", cost, {"model": model})

    def record_proxy_bytes(self, property_id: str, pms: str, bytes_used: int, rate_per_mb: float) -> None:
        """Record proxy bandwidth cost."""
        cost = (bytes_used / (1024 * 1024)) * rate_per_mb
        self._insert(property_id, pms, "", "proxy_mb", cost, {"bytes_used": bytes_used})

    def record_proxy_fetch(
        self, property_id: str, pms: str, proxy_tier: str, bytes_through_proxy: int, cost_usd: float
    ) -> None:
        """Record a proxy-tiered fetch."""
        self._insert(
            property_id,
            pms,
            proxy_tier,
            "proxy_mb",
            cost_usd,
            {"bytes_through_proxy": bytes_through_proxy, "proxy_tier": proxy_tier},
        )
    ...
    def _insert(self, property_id: str, pms: str, tier: str, category: str, cost: float, detail: dict[str, Any]) -> None:
        """Insert a cost entry."""
